...
**REDUCE Phase 開始前，必須驗證 MAP Phase 產出**：

```bash
./shared/tools/verify-perspectives.sh {type} {id} {expected_count}
```
...
│ Step 1: 評估總量                                                 │
│ 計算所有視角報告的總行數                                         │
│ total_lines = sum(wc -l perspectives/*.md)                      │
└─────────────────────────────────────────────────────────────────┘
         ↓
    總量 < 500 行？
         ↓
...
```bash
# 計算總行數
total=$(cat .claude/memory/{type}/{id}/perspectives/*.md | wc -l)
if [ "$total" -ge 500 ]; then
    echo "使用策略 B：並行子 Agent"
else
    echo "使用策略 A：直接讀取"
fi
```
...
|-----|------|------|
| 總量 < 500 行 | A | 可一次處理 |
| 總量 ≥ 500 行 | B | 需分散處理確保完整性 |
| 深度研究（--deep） | B | 報告通常較長 |
| 快速研究（--quick） | A | 報告較短 |
| 任何視角 > 300 行 | B | 單一大報告也建議分散處理 |
